...
const MusicList = () => {
   ...
   const removeSong = (songId) => {
      ...
      const updatedUser={...loggedUser,library:{...loggedUser.library,songs:newUserSongs},password:secureLocalStorage.getItem("loggedUserPassword")}
  ...
  return (
   ...
   <Grid container spacing={2} sx={{ flexWrap: "wrap" }}>
      {loggedUserMusic.map((song)=>{
         return <MusicCard key={Math.random()} songId={song} onClick={()=>removeSong(song)}/>
      })}
