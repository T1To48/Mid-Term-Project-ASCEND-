...
export default function Navbar() {
  const { loggedUser, setLoggedUser, localApi, setMusicSearchResult, theLogo } =
    useGlobalContext();
  ...
  useEffect(() => {
    setTimeout(() => {
      Object.keys(loggedUser).length > 0 ? setStatus(true) : setStatus(false);
    }, 1);
  }, [loggedUser]);
  ...
  const renderLibrary = (
    ...
    >
      <div style={{ marginTop: "1vh" }} onMouseLeave={handleLibraryMenuClose}>
        ...
        <MenuItem onClick={handleLibraryMenuClose}>
          <NavLink to="/music-library">Music</NavLink>
        </MenuItem>
      </div>
    </Menu>
  );
  ...
  return (
    <Box sx={{ flexGrow: 1, margin: "10%" }}>
      <AppBar
        position="fixed"
        sx={{ borderRadius: "20px", width: "80vw", margin: "1% 10%" }}
      >
        <Toolbar>
          <Typography variant="h6" noWrap component="div">
            <div
              onMouseOver={() => (theLogo.current.src = MainLogoOnHover)}
              onMouseLeave={() => (theLogo.current.src = MainLogo)}
